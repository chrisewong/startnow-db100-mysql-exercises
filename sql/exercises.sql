...
select distinct title, description, rating, length from film where length >= 180;
...
# 3b. Select the payment id, amount, and payment date columns from the payments table for payments made on or after 05/27/2005.
    select distinct payment_id, amount, payment_date from payment where payment_date >= 20050527;
